# Project: KDP Publishing Copilot
> Amazon KDP book publishing, cover design, layout

## How to Use
When starting a session in this project, read this file first.
When finishing work, append an entry below.

## Recent Changes

### 2026-08-16 — Chemo Journal Research Synthesis
- **What:** Deep research on competitor journals, clinical standards (UKONS, PRO-CTCAE, CR-UK), design best practices, and KDP market data. Consolidated into 9-section synthesis document.
- **Files:** `books/chemo-journal/RESEARCH_SYNTHESIS.md`
- **Impact:** Content spec validated against clinical standards; enhanced symptom list (18 UKONS symptoms), traffic light severity system, lab flag column, "what helped" per cycle, emergency triage guidance
- **Breaking changes:** None
- **Alerts for other projects:** None

### 2026-08-16 — Chemo Journal Publishing Playbook Created
- **What:** Full publishing playbook for Chemotherapy Treatment Journal (Book 2) — metadata, keywords, categories, description HTML, pricing, cover specs, upload checklist
- **Files:** `books/chemo-journal/PUBLISHING_PLAYBOOK.md`, `books/PIPELINE.md`
- **Impact:** Book 2 ready for cover design and KDP upload
- **Breaking changes:** None
- **Alerts for other projects:** None

### 2026-08-08 — Cover Dimensions Verified
- **What:** Cover dimensions confirmed: 6.25 × 9.25" (bleed), interior pages 6 × 9"
- **Files:** Affinity Publisher templates
- **Impact:** All pages fit within margin guides, no transparent bleed layer needed
- **Breaking changes:** None
- **Alerts for other projects:** None

## Pending Work
- [x] First book published (Notary Log — ASIN B0HDMKHB8Y)
- [ ] Chemo Journal — cover design, then KDP upload
- [ ] Password Log Book — interior build
- [ ] Running Log Book — interior build

## Active Book Status

| # | Book | Status | Interior | Cover | KDP |
|---|------|--------|----------|-------|-----|
| 1 | Notary Log | ✅ Live | ✅ | ✅ | ✅ Published |
| 2 | Chemo Journal | 🔨 Next | ✅ 130pp | ❌ Design needed | ❌ |
| 3 | Password Log | 📋 Planned | ❌ | ❌ | ❌ |
| 4 | Running Log | 📋 Planned | ❌ | ❌ | ❌ |

---
*Last updated: 2026-08-16*
